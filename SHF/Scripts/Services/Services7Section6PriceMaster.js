...
angular.module(config.app).service('Services7Section6PriceMasterCRUD', function ($http) {

    this.GetTableObject = function TableData() {
        let scope = angular.element(document.getElementById('Services7Section6PriceMasterControllerScope')).scope();
        let tenantId = scope.Services7Section6PriceMasterCreateOrEditViewModel.Tenant_ID == null ? 0 : scope.Services7Section6PriceMasterCreateOrEditViewModel.Tenant_ID;
        ...
        let oTable = $('#grdTable').DataTable({
            serverSide: true,
            ajax: {
                url: '/Post/Services7Section6Price/IndexAsync',
                type: 'POST',
                dataSrc: 'data',
            ...
            columns: [
                 ...
                 },
                {
                ...
                },
                 {
                  ...
                  {
                      name: "Services7Section6PriceMaster.SubSubCategoryName",
...
$('#grdTable tbody').on('click', '.btn-preview', function () {
            let rowData = oTable.row($(this).parents('tr')).data();
            let scope = angular.element(document.getElementById('Services7Section6PriceMasterControllerScope')).scope();
            scope.Preview('services7.html?u='+rowData.ServiceUrl+'#divSection6PriceMaster');
        });
    }
    ...
    this.LoadSection6IDByTenantAndSubSubCatID = function Section6IDByTenantAndSubSubCatID(tenantId,subsubcat_id) {
        let request = $http({
            method: "get",
            url: "/Get/Services7Section6PriceMaster/DropdownListbyTenantAsync?Id=" + tenantId+"&subsubcat_id="+subsubcat_id
        });
        return request;
    }
