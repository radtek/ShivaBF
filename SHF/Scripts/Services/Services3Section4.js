...
angular.module(config.app).service('Services3Section4CRUD', function ($http) {

    this.GetTableObject = function TableData() {
        let scope = angular.element(document.getElementById('Services3Section4ControllerScope')).scope();
        let tenantId = scope.Services3Section4CreateOrEditViewModel.Tenant_ID == null ? 0 : scope.Services3Section4CreateOrEditViewModel.Tenant_ID;
        ...
        let oTable = $('#grdTable').DataTable({
            serverSide: true,
            ajax: {
                url: '/Post/Services3Section4/IndexAsync',
                type: 'POST',
                dataSrc: 'data',
            ...
            columns: [
                 ...
                 },
                {
                 ...
                 {
                    name: "Services3Section4.SubSubCategoryName",
...

{
        ...
        $('#grdTable tbody').on('click', '.btn-preview', function () {
            let rowData = oTable.row($(this).parents('tr')).data();
            let scope = angular.element(document.getElementById('Services3Section4ControllerScope')).scope();
            scope.Preview('services3.html?u=' + rowData.ServiceUrl + '#divSection4');
        });
    }
