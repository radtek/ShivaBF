angular.module(config.app).controller('Services3MasterCtrl', ['$scope', '$http', '$window', 'SubSubCategoriesMasterCRUD', 'Services3MasterCRUD', 'TenantCRUD', 'CustomService', 'CodeValueCRUD', 'BannerMasterCRUD',
    function ($scope, $http, $window, SubSubCategoriesMasterCRUD, Services3MasterCRUD, TenantCRUD, CustomService, CodeValueCRUD, BannerMasterCRUD) {
        ...
        $scope.Entity = {};
        $scope.Services3MasterCreateOrEditViewModel = {};
        $scope.AllTenants = [];
 ...
 CKEDITOR.instances["Section1Description"].setData('');
            ...
            if ($scope.Cookie_Tenant_ID <= 0) {
                ...
                $scope.Services3MasterCreateOrEditViewModel.SelectedTenant_ID = -1;
               // $scope.Services3MasterCreateOrEditViewModel.SelectedUnitOfMesurment = -1;
            } else {
                $scope.Services3MasterCreateOrEditViewModel.Tenant_ID = $scope.Cookie_Tenant_ID;
               // $scope.BindUnitOfMeasurementDropDownList($scope.Services3MasterCreateOrEditViewModel.Tenant_ID);
            }
...
/********************************************************************************/
        $scope.EditAsync = function (Id) {
            ...
            $http.get("/Get/Services3Master/EditAsync?Id=" + Id
            ).then(
                function success(response) {
                    switch (response.data.Type) {
                        case 'Exception':
...
$scope.Services3MasterCreateOrEditViewModel.Section1Description= objEditorSection1.getData();
          ...
          $scope.Services3MasterCreateOrEditViewModel.ID == 0) ? "/Post/Services3Master/CreateAsync" : "/Post/Services3Master/EditAsync";
               $http.post($scope.path, $scope.Services3MasterCreateOrEditViewModel,
                    {
            ...
            swal({
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        var obj = {};
                        obj.Id = Id;
                  $http.post("/Post/Services3Master/Delete/", obj,
                    {
                        headers: { 'RequestVerificationToken': $scope.antiForgeryToken }
                    }
...
$scope.LoadAllSubSubCategory = function () {
            let tenantId = $scope.Services3MasterCreateOrEditViewModel.Tenant_ID;
debugger;
         ...
         function success(response) {
             switch (response.data.Type) {
                 case 'Exception':
                     ...
                     break;
                 case 'Response':
                     $scope.Services3MasterCreateOrEditViewModel.Url = response.data.Entity.Url;
                     console.clear();
                     break;
                 default:
                     CustomService.Notify(response.data.Message);
