angular.module(config.app).controller('CommentsReplyCtrl', ['$scope', '$http', '$window', 'SubSubCategoriesMasterCRUD', 'CommentsReplyCRUD', 'BlogMasterCRUD', 'TenantCRUD', 'CustomService', 'CodeValueCRUD', 'BlogCommentsDetailsCRUD',
    function ($scope, $http, $window, SubSubCategoriesMasterCRUD, CommentsReplyCRUD, BlogMasterCRUD, TenantCRUD, CustomService, CodeValueCRUD, BlogCommentsDetailsCRUD) {
        $scope.path = "";
        $scope.errors = {};
        ...
        $scope.CommentsReplyCreateOrEditViewModel.SelectedBlog_Id = -1;
        $scope.CommentsReplyCreateOrEditViewModel.SelectedBCD_Id = -1;
       
        $scope.Cookie_Tenant_ID = parseInt(CustomService.GetTenantID());
        $scope.CommentsReplyCreateOrEditViewModel.Tenant_ID = $scope.Cookie_Tenant_ID;     
       ...
       $scope.Guide = function () {
        ...
        $scope.CreateAsync = function () {
            ...
            if ($scope.Cookie_Tenant_ID <= 0) {
                ...
                $scope.CommentsReplyCreateOrEditViewModel.SelectedTenant_ID = -1;
               // $scope.CommentsReplyCreateOrEditViewModel.SelectedUnitOfMesurment = -1;
            } else {
                $scope.CommentsReplyCreateOrEditViewModel.Tenant_ID = $scope.Cookie_Tenant_ID;
               // $scope.BindUnitOfMeasurementDropDownList($scope.CommentsReplyCreateOrEditViewModel.Tenant_ID);
            }
...
/********************************************************************************/
        $scope.EditAsync = function (Id) {
            ...
            $http.get("/Get/CommentsReply/EditAsync?Id=" + Id
            ).then(
                function success(response) {
                    switch (response.data.Type) {
                        case 'Exception':
                            ...
                            break;
                        case 'Response':
                            $scope.CommentsReplyCreateOrEditViewModel = response.data.Entity;
                           // $scope.LoadAllCategory();
                           // $scope.LoadAllSubSubCategory();
                            //$scope.CommentsReplyCreateOrEditViewModel.Category_ID=$scope.CommentsReplyCreateOrEditViewModel.Category_ID;
                            $('#modal-createOredit').modal('show');
                            console.clear();
                            break;
                        case 'Validation':
        ...
        $scope.createOreditAsyncForm = function () {
           ...
           if ($scope.myForm.$valid) {
                $scope.path = ($scope.CommentsReplyCreateOrEditViewModel.ID == undefined || $scope.CommentsReplyCreateOrEditViewModel.ID == null || 
          $scope.CommentsReplyCreateOrEditViewModel.ID == 0) ? "/Post/CommentsReply/CreateAsync" : "/Post/CommentsReply/EditAsync";
               $http.post($scope.path, $scope.CommentsReplyCreateOrEditViewModel,
                    {
            ...
            swal({
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        var obj = {};
                        obj.Id = Id;
                  $http.post("/Post/CommentsReply/Delete/", obj,
                    {
                        headers: { 'RequestVerificationToken': $scope.antiForgeryToken }
                    }
                ...
                function success(response) {
                    switch (response.data.Type) {
                        case 'Exception':
                            ...
                            break;
                        case 'Response':
                            $scope.AllBlogComment = response.data.Entity;
                            console.clear();
                            break;
                        default:
                            CustomService.Notify(response.data.Message);
                ...
                function success(response) {
                    switch (response.data.Type) {
                        case 'Exception':
                            ...
                            break;
                        case 'Response':
                            $scope.CommentsReplyCreateOrEditViewModel.Url = response.data.Entity.Url;
                            console.clear();
                            break;
                        default:
                            CustomService.Notify(response.data.Message);
