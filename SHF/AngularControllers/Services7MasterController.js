angular.module(config.app).controller('Services7MasterCtrl', ['$scope', '$http', '$window', 'SubSubCategoriesMasterCRUD', 'Services7MasterCRUD', 'TenantCRUD', 'CustomService', 'CodeValueCRUD', 'BannerMasterCRUD',
    function ($scope, $http, $window, SubSubCategoriesMasterCRUD, Services7MasterCRUD, TenantCRUD, CustomService, CodeValueCRUD, BannerMasterCRUD) {
        $scope.path = "";
        $scope.errors = {};
                ...
                function success(response) {
                    switch (response.data.Type) {
                        ...
                        case 'Response':
                            ...
                            CKEDITOR.instances["Section1Description"].setData($scope.Services7MasterCreateOrEditViewModel.Section1Description);
...
$scope.Services7MasterCreateOrEditViewModel.Section1Description= objEditorSection1.getData();
        ...
        $scope.SelectBannerAsync = function (ID, BannerName) {
            $scope.Services7MasterCreateOrEditViewModel[$scope.SelectFor] = BannerName;
        ...
        $scope.SelectBannerasync = function (inputname) {
            ...
            $scope.AllBannerMaster = [];
            if ($scope.Services7MasterCreateOrEditViewModel.Tenant_ID == undefined || $scope.Services7MasterCreateOrEditViewModel.Tenant_ID <= 0 || $scope.Services7MasterCreateOrEditViewModel.Tenant_ID == null) {
                swal("Please select Tenant", "", "error");
                return;
            }
            var result = BannerMasterCRUD.LoadAllBannerMasterByTenantIdAsync($scope.Services7MasterCreateOrEditViewModel.Tenant_ID);
            result.then(
                function success(response) {
                    switch (response.data.Type) {
