angular.module(config.app).controller('IPInfoCtrl', ['$scope', '$http', '$window', 'IPInfoCRUD', 'TenantCRUD','CustomService',
    function ($scope, $http, $window, IPInfoCRUD, TenantCRUD,CustomService) {      
        $scope.path = "";
        $scope.errors = {};
        ...
        $scope.Entity = {};
        $scope.IPInfoCreateOrEditViewModel = {};
        $scope.AllTenants = [];
        
       
        $scope.IPInfoCreateOrEditViewModel.SelectedTenant_ID = -1;
        $scope.IPInfoCreateOrEditViewModel.SelectedUnitOfMesurment = -1;
        $scope.Cookie_Tenant_ID = parseInt(CustomService.GetTenantID());
        $scope.IPInfoCreateOrEditViewModel.Tenant_ID = $scope.Cookie_Tenant_ID;
        ...
        $scope.CreateAsync = function () {
            ...
            if ($scope.Cookie_Tenant_ID <= 0) {
                ...
                $scope.IPInfoCreateOrEditViewModel.Tenant_ID = $scope.Cookie_Tenant_ID;
                $scope.BindUnitOfMeasurementDropDownList($scope.IPInfoCreateOrEditViewModel.Tenant_ID);
            }
        ...
        $scope.EditAsync = function (Id) {
            ...
            $http.get("/Get/Banner/EditAsync?Id=" + Id
            ).then(
                function success(response) {
                    switch (response.data.Type) {
                        case 'Exception':
                            ...
                            break;
                        case 'Response':
                            $scope.IPInfoCreateOrEditViewModel = response.data.Entity;
                            $('#modal-createOredit').modal('show');
                            $scope.ImageProperty = {};
                            $scope.$apply();
                            $scope.fileList = [];
        ...
        $scope.createOreditAsyncForm = function () {
            ...
            if ($scope.myForm.$valid) {
                $scope.path = ($scope.IPInfoCreateOrEditViewModel.ID == undefined || $scope.IPInfoCreateOrEditViewModel.ID == null || $scope.IPInfoCreateOrEditViewModel.ID == 0) ? "/Post/Banner/CreateAsync" : "/Post/Banner/EditAsync";
                $http.post($scope.path, $scope.IPInfoCreateOrEditViewModel,
                    {
        ...
        $scope.UploadFileIndividual = function (fileToUpload, name, type, size, index) {
            var tenantId = $scope.IPInfoCreateOrEditViewModel.Tenant_ID;
            var reqObj = new XMLHttpRequest();
            reqObj.upload.addEventListener("progress", uploadProgress, false)
            reqObj.addEventListener("load", uploadComplete, false)
            reqObj.addEventListener("error", uploadFailed, false)
            reqObj.addEventListener("abort", uploadCanceled, false)
            reqObj.open("POST", "/POST/IPInfo/FileUpload", true);
            reqObj.setRequestHeader("Content-Type", "multipart/form-data");
            reqObj.setRequestHeader('X-File-Name', name);
            reqObj.setRequestHeader('X-File-Type', type);
            reqObj.setRequestHeader('X-File-Size', size);
            reqObj.setRequestHeader('tenantId', tenantId);
           $scope.IPInfoCreateOrEditViewModel.BannerPath=name;
