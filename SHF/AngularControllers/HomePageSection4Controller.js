angular.module(config.app).controller('HomePageSection4Ctrl', ['$scope', '$http', '$window','SubSubCategoriesMasterCRUD', 'HomePageSection4CRUD', 'TenantCRUD','CustomService','CodeValueCRUD','BannerMasterCRUD',
    function ($scope, $http, $window, SubSubCategoriesMasterCRUD, HomePageSection4CRUD, TenantCRUD, CustomService, CodeValueCRUD, BannerMasterCRUD) {
        $scope.path = "";
        $scope.errors = {};
        ...
        $scope.Entity = {};
        $scope.HomePageSection4CreateOrEditViewModel = {};
        $scope.AllTenants = [];
        $scope.AllSubSubCategories = [];
        $scope.HomePageSection4CreateOrEditViewModel.SelectedTenant_ID = -1;
        $scope.HomePageSection4CreateOrEditViewModel.SelectedSubSubCat_Id = -1;
       ...
       $scope.fileList = [];
        ...
        $scope.BindGrid = function () {
            HomePageSection4CRUD.LoadTable();
        }      
        ...
        $scope.CreateAsync = function () {
            ...
            if ($scope.Cookie_Tenant_ID <= 0) {
                ...
                $scope.HomePageSection4CreateOrEditViewModel.SelectedTenant_ID = -1;
               // $scope.HomePageSection4CreateOrEditViewModel.SelectedUnitOfMesurment = -1;
            } else {
                $scope.HomePageSection4CreateOrEditViewModel.Tenant_ID = $scope.Cookie_Tenant_ID;
               // $scope.BindUnitOfMeasurementDropDownList($scope.HomePageSection4CreateOrEditViewModel.Tenant_ID);
            }
...
/********************************************************************************/
        $scope.EditAsync = function (Id) {
            ...
            $http.get("/Get/HomePageSection4/EditAsync?Id=" + Id
            ).then(
                function success(response) {
                    switch (response.data.Type) {
                        case 'Exception':
        ...
        $scope.createOreditAsyncForm = function () {
           ...
           if ($scope.myForm.$valid) {
                $scope.path = ($scope.HomePageSection4CreateOrEditViewModel.ID == undefined || $scope.HomePageSection4CreateOrEditViewModel.ID == null || 
          $scope.HomePageSection4CreateOrEditViewModel.ID == 0) ? "/Post/HomePageSection4/CreateAsync" : "/Post/HomePageSection4/EditAsync";
               $http.post($scope.path, $scope.HomePageSection4CreateOrEditViewModel,
                    {
            ...
            swal({
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        var obj = {};
                        obj.Id = Id;
                  $http.post("/Post/HomePageSection4/Delete/", obj,
                    {
                        headers: { 'RequestVerificationToken': $scope.antiForgeryToken }
                    }
...
$scope.LoadAllSubSubCategory = function () {
            let tenantId = $scope.HomePageSection4CreateOrEditViewModel.Tenant_ID;
debugger;
            $scope.BindSubSubCategoryDropDownList(tenantId);
        }

$scope.BindSubSubCategoryDropDownList = function (tenantId) {
            let promise = HomePageSection4CRUD.LoadSubSubCategoriesDropdown(tenantId)
            promise.then(
                function success(response) {
                    switch (response.data.Type) {
                        case 'Exception':
 ...
 $scope.SelectBannerAsync = function (ID, BannerName) {
     $scope.HomePageSection4CreateOrEditViewModel[$scope.SelectFor] = BannerName;
 ...
 $scope.SelectBannerasync = function (inputname) {
     ...
     $scope.AllBannerMaster = [];
     if ($scope.HomePageSection4CreateOrEditViewModel.Tenant_ID == undefined || $scope.HomePageSection4CreateOrEditViewModel.Tenant_ID <= 0 || $scope.HomePageSection4CreateOrEditViewModel.Tenant_ID == null) {
         swal("Please select Tenant", "", "error");
         return;
     }
     var result = BannerMasterCRUD.LoadAllBannerMasterByTenantIdAsync($scope.HomePageSection4CreateOrEditViewModel.Tenant_ID);
     result.then(
         function success(response) {
             switch (response.data.Type) {
                ...
                function success(response) {
                    switch (response.data.Type) {
                        case 'Exception':
                            ...
                            break;
                        case 'Response':
                            $scope.HomePageSection4CreateOrEditViewModel.Url = response.data.Entity.Url;
                            console.clear();
                            break;
                        default:
                            CustomService.Notify(response.data.Message);
