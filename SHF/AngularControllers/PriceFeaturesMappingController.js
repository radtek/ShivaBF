angular.module(config.app).controller('PriceFeaturesMappingCtrl', ['$scope', '$http', '$window', 'PriceFeaturesMappingCRUD','PriceFeaturesMasterCRUD', 'TenantCRUD','CustomService',
    function ($scope, $http, $window, PriceFeaturesMappingCRUD,PriceFeaturesMasterCRUD, TenantCRUD,CustomService) {      
        $scope.path = "";
        $scope.errors = {};
...
let subsubcat_id = $scope.PriceFeaturesMappingCreateOrEditViewModel.SubSubCat_Id;
            $scope.BindSubSubCategoryDropDownList(tenantId,subsubcat_id);
        }
...
$scope.LoadAllPriceFeaturesMaster = function () {
            ...
            let tenantId = $scope.PriceFeaturesMappingCreateOrEditViewModel.Tenant_ID;
          $scope.PriceFeaturesMaster(tenantId);
        }
