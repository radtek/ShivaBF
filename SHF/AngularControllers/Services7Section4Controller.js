angular.module(config.app).controller('Services7Section4Ctrl', ['$scope', '$http', '$window','CategoriesMasterCRUD','SubCategoriesMasterCRUD', 'Services7Section4CRUD','Services7MasterCRUD', 'TenantCRUD','CustomService','CodeValueCRUD',
    function ($scope, $http, $window,CategoriesMasterCRUD,SubCategoriesMasterCRUD, Services7Section4CRUD,Services7MasterCRUD, TenantCRUD,CustomService,CodeValueCRUD) {      
        $scope.path = "";
        $scope.errors = {};
        ...
        $scope.Entity = {};
        $scope.Services7Section4CreateOrEditViewModel = {};
        $scope.AllTenants = [];
        ...
        $scope.BindGrid = function () {
            Services7Section4CRUD.LoadTable();
        }      
        ...
        $scope.CreateAsync = function () {
            ...
            if ($scope.Cookie_Tenant_ID <= 0) {
                ...
                $scope.Services7Section4CreateOrEditViewModel.SelectedTenant_ID = -1;
               // $scope.Services7Section4CreateOrEditViewModel.SelectedUnitOfMesurment = -1;
            } else {
                $scope.Services7Section4CreateOrEditViewModel.Tenant_ID = $scope.Cookie_Tenant_ID;
               // $scope.BindUnitOfMeasurementDropDownList($scope.Services7Section4CreateOrEditViewModel.Tenant_ID);
            }
...
/********************************************************************************/
        $scope.EditAsync = function (Id) {
            ...
            $http.get("/Get/Services7Section4/EditAsync?Id=" + Id
            ).then(
                function success(response) {
                    switch (response.data.Type) {
                        case 'Exception':
                            ...
                            break;
                        case 'Response':
                            $scope.Services7Section4CreateOrEditViewModel = response.data.Entity;
                           // $scope.LoadAllCategory();
                            $scope.LoadAllSubSubCategory();
                            //$scope.Services7Section4CreateOrEditViewModel.Category_ID=$scope.Services7Section4CreateOrEditViewModel.Category_ID;
                            $('#modal-createOredit').modal('show');
                            console.clear();
                            break;
                        case 'Validation':
        ...
        $scope.createOreditAsyncForm = function () {
           ...
           if ($scope.myForm.$valid) {
                $scope.path = ($scope.Services7Section4CreateOrEditViewModel.ID == undefined || $scope.Services7Section4CreateOrEditViewModel.ID == null || 
          $scope.Services7Section4CreateOrEditViewModel.ID == 0) ? "/Post/Services7Section4/CreateAsync" : "/Post/Services7Section4/EditAsync";
               $http.post($scope.path, $scope.Services7Section4CreateOrEditViewModel,
                    {
            ...
            swal({
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        var obj = {};
                        obj.Id = Id;
                  $http.post("/Post/Services7Section4/Delete/", obj,
                    {
                        headers: { 'RequestVerificationToken': $scope.antiForgeryToken }
                    }
...
$scope.LoadAllSubSubCategory = function () {
            let tenantId = $scope.Services7Section4CreateOrEditViewModel.Tenant_ID;
