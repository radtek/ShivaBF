angular.module(config.app).controller('Services3Section6PriceMasterCtrl', ['$scope', '$http', '$window','CategoriesMasterCRUD','SubCategoriesMasterCRUD', 'Services3Section6PriceMasterCRUD','Services3MasterCRUD','TenantCRUD','CustomService','CodeValueCRUD',
    function ($scope, $http, $window,CategoriesMasterCRUD,SubCategoriesMasterCRUD, Services3Section6PriceMasterCRUD,Services3MasterCRUD, TenantCRUD,CustomService,CodeValueCRUD) {      
        $scope.path = "";
        $scope.errors = {};
 ...
 $scope.Services3Section6PriceMasterCreateOrEditViewModel.SelectedState_Id = -1;
       ...
       $scope.Guide = function () {
        ...
        $scope.BindGrid = function () {
            Services3Section6PriceMasterCRUD.LoadTable();
        }      
        ...
        $scope.CreateAsync = function () {
            ...
            if ($scope.Cookie_Tenant_ID <= 0) {
                ...
                $scope.Services3Section6PriceMasterCreateOrEditViewModel.SelectedTenant_ID = -1;
               // $scope.Services3Section6PriceMasterCreateOrEditViewModel.SelectedUnitOfMesurment = -1;
            } else {
                $scope.Services3Section6PriceMasterCreateOrEditViewModel.Tenant_ID = $scope.Cookie_Tenant_ID;
               // $scope.BindUnitOfMeasurementDropDownList($scope.Services3Section6PriceMasterCreateOrEditViewModel.Tenant_ID);
            }
...
/********************************************************************************/
        $scope.EditAsync = function (Id) {
            ...
            $http.get("/Get/Services3Section6PriceMaster/EditAsync?Id=" + Id
            ).then(
                function success(response) {
                    switch (response.data.Type) {
                        case 'Exception':
 ...
 $scope.LoadStates();
        ...
        $scope.createOreditAsyncForm = function () {
           ...
           if ($scope.myForm.$valid) {
                $scope.path = ($scope.Services3Section6PriceMasterCreateOrEditViewModel.ID == undefined || $scope.Services3Section6PriceMasterCreateOrEditViewModel.ID == null || 
          $scope.Services3Section6PriceMasterCreateOrEditViewModel.ID == 0) ? "/Post/Services3Section6PriceMaster/CreateAsync" : "/Post/Services3Section6PriceMaster/EditAsync";
               $http.post($scope.path, $scope.Services3Section6PriceMasterCreateOrEditViewModel,
                    {
            ...
            swal({
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        var obj = {};
                        obj.Id = Id;
                  $http.post("/Post/Services3Section6PriceMaster/Delete/", obj,
                    {
                        headers: { 'RequestVerificationToken': $scope.antiForgeryToken }
                    }
...
$scope.LoadAllSubSubCategory = function () {
            let tenantId = $scope.Services3Section6PriceMasterCreateOrEditViewModel.Tenant_ID;
