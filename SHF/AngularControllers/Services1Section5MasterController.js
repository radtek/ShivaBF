angular.module(config.app).controller('Services1Section5MasterCtrl', ['$scope', '$http', '$window','CategoriesMasterCRUD','SubCategoriesMasterCRUD', 'Services1Section5MasterCRUD','Services1MasterCRUD', 'TenantCRUD','CustomService','CodeValueCRUD',
    function ($scope, $http, $window,CategoriesMasterCRUD,SubCategoriesMasterCRUD, Services1Section5MasterCRUD,Services1MasterCRUD, TenantCRUD,CustomService,CodeValueCRUD) {      
        ...
        $scope.Entity = {};
        $scope.Services1Section5MasterCreateOrEditViewModel = {};
        $scope.AllTenants = [];
        ...
        $scope.Services1Section5MasterCreateOrEditViewModel.Tenant_ID = $scope.Cookie_Tenant_ID;     
       ...
       $scope.Guide = function () {
        ...
        $scope.BindGrid = function () {
            Services1Section5MasterCRUD.LoadTable();
        }      
        ...
        $scope.CreateAsync = function () {
            ...
            if ($scope.Cookie_Tenant_ID <= 0) {
                ...
                $scope.Services1Section5MasterCreateOrEditViewModel.SelectedTenant_ID = -1;
               // $scope.Services1Section5MasterCreateOrEditViewModel.SelectedUnitOfMesurment = -1;
            } else {
                $scope.Services1Section5MasterCreateOrEditViewModel.Tenant_ID = $scope.Cookie_Tenant_ID;
               // $scope.BindUnitOfMeasurementDropDownList($scope.Services1Section5MasterCreateOrEditViewModel.Tenant_ID);
            }
...
/********************************************************************************/
        $scope.EditAsync = function (Id) {
            ...
            $http.get("/Get/Services1Section5Master/EditAsync?Id=" + Id
            ).then(
                function success(response) {
                    switch (response.data.Type) {
                        case 'Exception':
                            ...
                            break;
                        case 'Response':
                            $scope.Services1Section5MasterCreateOrEditViewModel = response.data.Entity;
 CKEDITOR.instances["ShortDescription"].setData($scope.Services1Section5MasterCreateOrEditViewModel.ShortDescription);
                           // $scope.LoadAllCategory();
                            $scope.LoadAllSubSubCategory();
                            //$scope.Services1Section5MasterCreateOrEditViewModel.Category_ID=$scope.Services1Section5MasterCreateOrEditViewModel.Category_ID;
                            $('#modal-createOredit').modal('show');
                            console.clear();
                            break;
                        case 'Validation':
        ...
        $scope.createOreditAsyncForm = function () {
          ...
          var objEditorShort = CKEDITOR.instances["ShortDescription"];
           $scope.Services1Section5MasterCreateOrEditViewModel.ShortDescription= objEditorShort.getData();
           if ($scope.myForm.$valid) {
                $scope.path = ($scope.Services1Section5MasterCreateOrEditViewModel.ID == undefined || $scope.Services1Section5MasterCreateOrEditViewModel.ID == null || 
          $scope.Services1Section5MasterCreateOrEditViewModel.ID == 0) ? "/Post/Services1Section5Master/CreateAsync" : "/Post/Services1Section5Master/EditAsync";
               $http.post($scope.path, $scope.Services1Section5MasterCreateOrEditViewModel,
                    {
            ...
            swal({
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        var obj = {};
                        obj.Id = Id;
                  $http.post("/Post/Services1Section5Master/Delete/", obj,
                    {
                        headers: { 'RequestVerificationToken': $scope.antiForgeryToken }
                    }
