...
namespace SHF.Controllers
{
    ...
    public class Services1Section1MasterController : BaseController
    {
        ...
        [Route("Configurations/Master/ServiceType1/Section1")]
        [Route("Settings/Master/ServiceType1/Section1")]
                ...
                else
                {
                    using (var transaction = new TransactionScope())
                    {
                        try
                        {
                            ...
                            {
                                var entityServices = this.businessServices1Master.FindBy(services1master => services1master.SubSubCat_Id == model.SubSubCat_Id).FirstOrDefault();
                                var entity = this.businessServices1Section1Master.GetById(Convert.ToInt64(model.ID));
                                if (entity.IsNotNull())
                                {
            ...
            try
            {
                using (var transaction = new TransactionScope(TransactionScopeOption.Required, new TransactionOptions() { IsolationLevel = IsolationLevel.ReadUncommitted }))
                {
                    try
                    {
                        ...
                        {
                            var entities = this.businessServices1Section1Master.FindBy(product => product.Tenant_ID == Id).Select(x => new ViewModel.Services1Section1MasterDropdownListViewModel
                            {
                                ID = x.ID,
                            ...
                            {
                                var response = new JsonResponse<IEnumerable<ViewModel.Services1Section1MasterDropdownListViewModel>>()
                                {
                                    Type = busConstant.Messages.Type.RESPONSE,
                                    Entity = entities
                                };
