...
{
    [AllowAnonymous]
    public class Services8MasterController : BaseController
    {
        ...
        public Services8MasterController(Business.Interface.IMessage Imessage, Business.Interface.IServices8Master IServices8Master, Business.Interface.ITenant Itenant, Business.Interface.ISubSubCategoriesMaster IsubSubCategoriesMaster)
        {
        ...
        [Route("Configurations/Master/ServiceType8/Index")]
        [Route("Settings/Master/ServiceType8/Index")]
        ...
        [HttpPost]
        [Route("Post/ServiceType8/IndexAsync")]
        [ValidateAntiForgeryTokens]
        public async Task<ActionResult> IndexAsync()
        {
            BusinessResultViewModel<ViewModel.Services8MasterIndexViewModel> businessResult;
            try
            {
                using (var transaction = new TransactionScope(TransactionScopeOption.Required, new TransactionOptions() { IsolationLevel = IsolationLevel.ReadUncommitted }))
                {
                    try
                    {
                        ...
                        businessResult = this.businessServices8Master.Index(Request, tenantId);
        ...
        [AuditAttribute]
        [ValidateAntiForgeryTokens]
        [Route("Post/Services8Master/CreateAsync")]
        public async Task<ActionResult> CreateAsync(ViewModel.Services8MasterCreateOrEditViewModel model)
        {
            try
            {
                ...
                else
                {
                    using (var transaction = new TransactionScope())
                    {
                        try
                        {
                            var catId = businessServices8Master.FindBy(Categories => Categories.Tenant_ID == model.Tenant_ID && Categories.SubSubCat_Id == model.SubSubCat_Id).FirstOrDefault();
                            ...
                            else
                            {
                                ...
                                var entity = new EntityModel.Services8Master();
                                ...
                                entity.CategoriesMaster = null;
                                this.businessServices8Master.Create(entity);
                                transaction.Complete();
        ...
        [HttpGet]
        [Route("Get/Services8Master/EditAsync")]
            ...
            try
            {
                using (var transaction = new TransactionScope(TransactionScopeOption.Required, new TransactionOptions() { IsolationLevel = IsolationLevel.ReadUncommitted }))
                {
                    try
                    {
                        ...
                        {
                            var entity = this.businessServices8Master.GetById(Id);

                            if (entity.IsNotNull())
                            {
                                var model = new ViewModel.Services8MasterCreateOrEditViewModel();
                                ...
                                var response = new JsonResponse<Services8MasterCreateOrEditViewModel>()
                                {
                                    Type = busConstant.Messages.Type.RESPONSE,
                                    Entity = model
                                };
        ...
        [AuditAttribute]
        [ValidateAntiForgeryTokens]
        [Route("Post/Services8Master/EditAsync")]
        public async Task<ActionResult> EditAsync(ViewModel.Services8MasterCreateOrEditViewModel model)
        {
            try
            {
                ...
                else
                {
                    using (var transaction = new TransactionScope())
                    {
                        try
                        {
                            var CategoriesData = businessServices8Master.FindBy(Categories => Categories.Tenant_ID == model.Tenant_ID && Categories.ID == model.ID).FirstOrDefault();
                            ...
                            else
                            {
                                ...
                                var entity = this.businessServices8Master.GetById(Convert.ToInt64(model.ID));
                                if (entity.IsNotNull())
                                {
                                    entity.BannerImagePath = model.BannerImagePath;
                                    entity.BannerOnHeading = model.BannerOnHeading;
                                    ...
                                    this.businessServices8Master.Update(entity);
        ...
        [AuditAttribute]
        [ValidateAntiForgeryTokens]
        [Route("Post/Services8Master/Delete")]
            ...
            try
            {
                using (var transaction = new TransactionScope(TransactionScopeOption.Required, new TransactionOptions() { IsolationLevel = IsolationLevel.ReadUncommitted }))
                {
                    try
                    {
                        ...
                        {
                            this.businessServices8Master.Delete(Convert.ToInt64(Id));
        ...
        [HttpGet]
        [Route("Get/Services8Master/DropdownListbyTenantAsync")]
        public async Task<ActionResult> GetServices8MasterByTenantIdAsync(long? Id)
        {
            try
            {
                using (var transaction = new TransactionScope(TransactionScopeOption.Required, new TransactionOptions() { IsolationLevel = IsolationLevel.ReadUncommitted }))
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            var entities = this.businessSubSubCategoriesMaster.GetAll().Where(x => (x.Tenant_ID == Id && x.ServiceTypeValue == busConstant.Code.CodeValue.ServiceType.SERVICE_8))
                                .Select(x => new ViewModel.Services8MasterDropdownListViewModel
                                {
                                    ID = x.ID,
                            ...
                            {
                                var response = new JsonResponse<IEnumerable<ViewModel.Services8MasterDropdownListViewModel>>()
                                {
                                    Type = busConstant.Messages.Type.RESPONSE,
                                    Entity = entities
                                };
