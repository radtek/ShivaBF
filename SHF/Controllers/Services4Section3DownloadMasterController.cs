...
{
    [AllowAnonymous]
    public class Services4Section3DownloadMasterController : BaseController
    {
        ...
        [Route("Configurations/Master/ServiceType4/Section3DownloadMaster")]
        [Route("Settings/Master/ServiceType4/Section3DownloadMaster")]
        ...
        [HttpPost]
        [Route("Post/Services4Section3Download/IndexAsync")]
        [ValidateAntiForgeryTokens]
        public async Task<ActionResult> IndexAsync()
        {
            BusinessResultViewModel<ViewModel.Services4Section3DownloadMasterIndexViewModel> businessResult;
            try
            {
                using (var transaction = new TransactionScope(TransactionScopeOption.Required, new TransactionOptions() { IsolationLevel = IsolationLevel.ReadUncommitted }))
                {
                    try
                    {
                        ...
                        businessResult = this.businessServices4Section3DownloadMaster.Index(Request, tenantId);
        ...
        [AuditAttribute]
        [ValidateAntiForgeryTokens]
        [Route("Post/Services4Section3DownloadMaster/CreateAsync")]
        public async Task<ActionResult> CreateAsync(ViewModel.Services4Section3DownloadMasterCreateOrEditViewModel model)
        {
            try
            {
                ...
                else
                {
                    using (var transaction = new TransactionScope())
                    {
                        try
                        {
                            var productId = businessServices4Section3DownloadMaster.FindBy(Services4Section3Download => Services4Section3Download.Tenant_ID == model.Tenant_ID && Services4Section3Download.ID == model.ID).FirstOrDefault();
                            ...
                            else
                            {
                                ...
                                this.businessServices4Section3DownloadMaster.Create(entity);
                                transaction.Complete();
        ...
        [HttpGet]
        [Route("Get/Services4Section3DownloadMaster/EditAsync")]
            ...
            try
            {
                using (var transaction = new TransactionScope(TransactionScopeOption.Required, new TransactionOptions() { IsolationLevel = IsolationLevel.ReadUncommitted }))
                {
                    try
                    {
                        ...
                        {
                            var entity = this.businessServices4Section3DownloadMaster.GetById(Id);
                            ...
                            if (entity.IsNotNull())
                            {
                                ...
                                var response = new JsonResponse<Services4Section3DownloadMasterCreateOrEditViewModel>()
                                {
                                    Type = busConstant.Messages.Type.RESPONSE,
                                    Entity = model
                                };
        ...
        [AuditAttribute]
        [ValidateAntiForgeryTokens]
        [Route("Post/Services4Section3DownloadMaster/EditAsync")]
        public async Task<ActionResult> EditAsync(ViewModel.Services4Section3DownloadMasterCreateOrEditViewModel model)
        {
            try
            {
                ...
                else
                {
                    using (var transaction = new TransactionScope())
                    {
                        try
                        {
                            var Services4Section3DownloadData = businessServices4Section3DownloadMaster.FindBy(Services4Section3Download => Services4Section3Download.Tenant_ID == model.Tenant_ID && Services4Section3Download.ID == model.ID).FirstOrDefault();

                            if (Services4Section3DownloadData.IsNull())
                            {
                                transaction.Complete();
                                var response = new JsonResponse<dynamic>()
                                {
                            ...
                            {
                                var entityServices = this.businessServices4Master.FindBy(Services4Master => Services4Master.SubSubCat_Id == model.SubSubCat_Id).FirstOrDefault();
                                var entity = this.businessServices4Section3DownloadMaster.GetById(Convert.ToInt64(model.ID));
                                if (entity.IsNotNull())
                                {
                                    entity.Service_Id = entityServices.ID;
                                    entity.SubSubCat_Id = model.SubSubCat_Id;
        ...
        [AuditAttribute]
        [ValidateAntiForgeryTokens]
        [Route("Post/Services4Section3DownloadMaster/Delete")]
            ...
            try
            {
                using (var transaction = new TransactionScope(TransactionScopeOption.Required, new TransactionOptions() { IsolationLevel = IsolationLevel.ReadUncommitted }))
                {
                    try
                    {
                        ...
                        {
                            this.businessServices4Section3DownloadMaster.Delete(Convert.ToInt64(Id));
        ...
        [HttpGet]
        [Route("Get/Services4Section3DownloadMaster/DropdownListbyTenantAsync")]
        public async Task<ActionResult> GetServices4Section3DownloadMasterByTenantIdAsync(long Id)
        {
            try
            {
                using (var transaction = new TransactionScope(TransactionScopeOption.Required, new TransactionOptions() { IsolationLevel = IsolationLevel.ReadUncommitted }))
                {
                    try
                    {
                        ...
                        {
                            var entities = this.businessServices4Section3DownloadMaster.FindBy(product => product.Tenant_ID == Id).Select(x => new ViewModel.Services4Section3DownloadMasterDropdownListViewModel
                            {
                                ID = x.ID,
                            ...
                            {
                                var response = new JsonResponse<IEnumerable<ViewModel.Services4Section3DownloadMasterDropdownListViewModel>>()
                                {
                                    Type = busConstant.Messages.Type.RESPONSE,
                                    Entity = entities
                                };
