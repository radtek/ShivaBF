INSERT INTO [dbo].[Tbl_CategoriesMaster]
   ...
   select distinct dbo.udf_TitleCase(MAIN_CATEGORY_MENU) as MAIN_CATEGORY_MENU,1,1,1,10,
   ...
   insert into [dbo].[Tbl_SubCategoriesMaster]
      ...
      ,[Modified_On]
      ,[Is_Deleted])
    select distinct dbo.udf_TitleCase(t.sub_category_menu),c.ID,1,1,1,10,'/'+lower(replace(replace(dbo.udf_TitleCase(t.sub_category_menu),' ','_'),'&','')),
   ...
   insert into [dbo].[Tbl_SubSubCategoriesMaster](
      ...
      ,[Modified_On]
      ,[Is_Deleted])
	  select distinct dbo.udf_TitleCase(t.service_name),c.ID,s.ID,1,1,1,'1020',case 
