IF EXISTS (SELECT * FROM sys.objects WHERE name = 'usp_DeleteService3')  
DROP PROCEDURE dbo.usp_DeleteService3  
GO
CREATE PROCEDURE [dbo].[usp_DeleteService3]
(
@serviceId BIGINT=NULL
)
AS
BEGIN
DECLARE @NumDeleted AS int;
    SET @NumDeleted = 0;
	SET NOCOUNT ON;
	 BEGIN TRANSACTION ucDelCand
	delete from Tbl_Services3HeadingButtons where Service_Id=@serviceId;
	delete from Tbl_Services3Section4 where Service_Id=@serviceId;
	delete from Tbl_Services3Section6PriceMaster where Service_Id=@serviceId;
	delete from Tbl_Services3Master where ID=@serviceId;
	SET @NumDeleted = @@ROWCOUNT;

    COMMIT TRANSACTION ucDelCand

    SELECT @NumDeleted;
END
Go